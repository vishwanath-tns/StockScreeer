...
class PlanetaryPositionViewer:
    ...
    def setup_ui(self):
        ...
        self.subtitle_label = tk.Label(
            title_frame,
            text=f"Database Browser • {self.date_range_text} • Verify with DrikPanchang & J.Hora",
    ...
    def setup_control_panel(self, parent):
        ...
        self.year_combo.pack(side=tk.LEFT, padx=(5, 0))
        ...
        self.month_combo = ttk.Combobox(month_frame, textvariable=self.month_var, 
                                  values=[str(i) for i in range(1, 12)], width=8, state="readonly")
        self.month_combo.pack(side=tk.LEFT, padx=(5, 0))
    ...
    def setup_position_tab(self):
        ...
        for i, (planet, symbol, color) in enumerate(planets):
            ...
            card_frame = tk.Frame(planets_frame, bg=color, relief=tk.RAISED, bd=2, width=240, height=150)
            card_frame.grid(row=row, column=col, padx=5, pady=8, sticky='nsew')
            card_frame.grid_propagate(False)  # Prevent frame from shrinking
            
            # Configure grid weights
            planets_frame.grid_rowconfigure(row, weight=1, minsize=160)
            planets_frame.grid_columnconfigure(col, weight=1, minsize=250)
            ...
            data_frame.pack(fill=tk.BOTH, expand=True, padx=3, pady=(0, 5))
            
            self.planet_cards[planet.lower()] = {
                'longitude': tk.Label(data_frame, text="Long: --", font=self.fonts['small'], bg='white', anchor='w'),
                'sign': tk.Label(data_frame, text="Sign: --", font=self.fonts['small'], bg='white', anchor='w'),
                'degree': tk.Label(data_frame, text="Deg: --", font=self.fonts['small'], bg='white', anchor='w')
            }
            
            self.planet_cards[planet.lower()]['longitude'].pack(anchor='w', padx=5, pady=(3, 1), fill='x')
            self.planet_cards[planet.lower()]['sign'].pack(anchor='w', padx=5, pady=1, fill='x')
            self.planet_cards[planet.lower()]['degree'].pack(anchor='w', padx=5, pady=(1, 3), fill='x')
    ...
    def load_available_dates(self):
        """Load available years and months from database"""
        try:
            conn = pymysql.connect(**self.mysql_config)
            cursor = conn.cursor()
            
            # Get available years
            cursor.execute("SELECT DISTINCT year FROM planetary_positions ORDER BY year")
            self.available_years = [str(row[0]) for row in cursor.fetchall()]
            
            # Get available months for each year
            self.available_months = {}
            for year in self.available_years:
                cursor.execute("SELECT DISTINCT month FROM planetary_positions WHERE year = %s ORDER BY month", (year,))
                self.available_months[year] = [str(row[0]) for row in cursor.fetchall()]
            ...
            if date_range[0] and date_range[1]:
                ...
                self.date_range_text = f"{min_date.strftime('%b %Y')} to {max_date.strftime('%b %Y')}"
            else:
                self.date_range_text = "No data available"
            
            conn.close()
            
            # Update the UI
            self.update_date_dropdowns()
            self.update_subtitle()
            
        except Exception as e:
            self.date_range_text = f"Error loading dates: {str(e)}"
            self.available_years = ["2024"]  # Fallback
            self.available_months = {"2024": ["1", "2", "3", "4", "5", "6"]}
    
    def update_date_dropdowns(self):
        """Update the dropdown values with available dates"""
        if hasattr(self, 'year_combo'):
            # Update year dropdown
            self.year_combo['values'] = self.available_years
            if self.available_years:
                # Set to the most recent year
                current_year = max(self.available_years)
                self.year_var.set(current_year)
                
                # Update months for current year
                if current_year in self.available_months:
                    self.month_combo['values'] = self.available_months[current_year]
                    if self.available_months[current_year]:
                        self.month_var.set(self.available_months[current_year][-1])  # Most recent month
    
    def update_subtitle(self):
        """Update the subtitle with current date range"""
    ...
    def on_year_change(self, event=None):
        ...
        try:
            ...
            if selected_year in self.available_months:
                available_months = self.available_months[selected_year]
                self.month_combo['values'] = available_months
                if available_months:
                    self.month_var.set(available_months[0])  # First available month
                    self.on_month_change()  # Update days
        except Exception as e:
            print(f"Error updating months: {e}")
            ...
            if planet in self.planet_cards:
                ...
                if longitude is not None:
                    self.planet_cards[planet]['longitude'].config(
                        text=f"Long: {longitude:.4f}°"
                    )
                    self.planet_cards[planet]['sign'].config(
                        text=f"Sign: {sign}"
                    )
                    self.planet_cards[planet]['degree'].config(
                        text=f"Deg: {degree:.4f}°"
                    )
                else:
                    self.planet_cards[planet]['longitude'].config(text="Long: No data")
...
"""
    ...
    def clear_positions(self):
        ...
        for planet in self.planet_cards:
            self.planet_cards[planet]['longitude'].config(text="Long: --")
            self.planet_cards[planet]['sign'].config(text="Sign: --")
            self.planet_cards[planet]['degree'].config(text="Deg: --")
